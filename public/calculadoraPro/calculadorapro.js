...
let resultados = [];
let limpiarResultados = () =>{ resultados.splice(0, resultados.length);
};
function calculatorPro() {
    ...
    do {
      nuevoNumero = prompt('Introduce varios números y realizaré la suma, resta, producto y división entre ellos. Si introduces solo un número y se calculará su raiz. Pulsa cancelar cuando no quieras introducir mas. ');
      if(!isNaN(nuevoNumero)){
        listaNumeros.push(Number(nuevoNumero));
        
      } else {
          alert ("Debes introducir solo números y usar un punto para los decimales. ");
      }
  ...
  if(listaNumeros.length === 1){
    let raizCuadrada = Math.sqrt(parseFloat(listaNumeros[0]));
    ...
    console.log("La raiz cuadrada es: " + raizCuadradaDec); 
      ...
      resultados.push(sumaTotal.toFixed(3), restaTotal.toFixed(3), multiplicacionTotal.toFixed(3), divisionTotal.toFixed(3))
}
...
calculatorPro();
  ...
  let otraOperacion = prompt("Deseas realizar otra operación? Porfavor, respon si o no .")
